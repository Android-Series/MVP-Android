...
public class Presenter implements DataBusSubscriber {
    ...
    public void getUser() {
        manager.getUser(); //通过manager对象向下传递指令：一层一层的往下传递命令
    }
    ...
    public void onEvent(Object data) {
        if(data instanceof User){
            Log.d(RxLineTest.TAG,"receive User in thread:"+Thread.currentThread());
        }else if(data instanceof Order){
            Log.d(RxLineTest.TAG,"receiver Order :"+Thread.currentThread());
        }else {
            Log.d(RxLineTest.TAG,"receiver data :"+Thread.currentThread());
        }
    }
}
